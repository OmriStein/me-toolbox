...
- [ ] `me_toolbox/gears/transmission.py` `ZE` property — swallows a `TypeError` (raised when a
  gear's `material` string isn't in the known elastic-modulus table) and silently returns
  `None` instead of raising. Same family of bug as `Kb`/`YN` above. Found and pinned down as
  current behavior while writing `test_transmission.py`.
...
- [x] `icecream` is imported by `fatigue_analysis.py` but declared in neither
  `requirements.txt` nor `setup.py` — fresh installs broke on import. Added to both.
...
- [x] Every previously-untested class now has tests: `fatigue` (`EnduranceLimit`,
  `FatigueAnalysis`, `FailureCriteria` — 67 tests), `ExtensionSpring`/`HelicalTorsionSpring` (61
  tests), `gears` (`Gear`, `SpurGear`, `HelicalGear`, `Transmission` — 118 tests), `BoltPattern`
  (30 tests). Whole repo: 321 tests, all passing. All expected values were generated by
  actually running the current code (never invented), independently re-deriving closed-form
  formulas inline in assertions wherever natural, per instruction to trust current code over
  stale test expectations.
  - Surfaced along the way (logged separately above/below where relevant): the two
    `ExtensionSpring` bugs (`weight`/`__repr__`), and `Transmission.ZE` silently returning
    `None` instead of raising for an unrecognized gear material (documented as current
    behavior via `test_ZE_returns_none_for_invalid_material` in `test_transmission.py`, not
    fixed — decide if that should actually raise).
